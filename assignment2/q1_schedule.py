...
class LinearSchedule(object):
    ...
    def update(self, t):
        ...
        self.epsilon = np.linspace(self.eps_begin, self.eps_end, self.nsteps+1)[t] if t<=self.nsteps else self.eps_end
        ##############################################################
        ######################## END YOUR CODE ############## ########
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action):
        ...
        return best_action if np.random.rand()>self.epsilon else np.random.randint(self.env.action_space.n)
        ##############################################################
        ######################## END YOUR CODE #######################
